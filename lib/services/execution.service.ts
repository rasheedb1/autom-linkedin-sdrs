...
import type { StepType, CadenceLead, CadenceStep, LeadStepInstance, Lead } from '../types/database';
...
export class ExecutionService {
  ...
  async executeStep(
    ...
    const { data: cadenceLead, error: clError } = await this.supabase
      .from('cadence_leads')
      .select(`
        *,
        lead:leads(*)
  ...
  async executeNextStep(ownerId: string, cadenceLeadId: string): Promise<ExecuteStepResult> {
    ...
    const { data: cadenceLead, error } = await this.supabase
      .from('cadence_leads')
  ...
  async executeBatch(
    ...
    const { data: cadenceLeads, error } = await this.supabase
      .from('cadence_leads')
      .select('*')
  ...
  async getPendingLeads(
    ...
    const { data: cadenceLeads } = await this.supabase
      .from('cadence_leads')
      .select(`
        *,
        lead:leads(*)
